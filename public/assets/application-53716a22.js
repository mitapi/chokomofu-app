...
var snack_reaction_controller_default = class extends Controller {
  ...
  connect() {
    ...
    this._closing = false;
    this._tick = false;
    console.log("[snack-reaction] connected keys=", Object.keys(this.expressionMapValue || {}));
    this._onStepChange = (e) => {
      const currentLine = this.messageTarget?.querySelector('[data-conversation-step-target="line"]:not(.hidden)');
      const key = currentLine?.dataset?.expressionKey;
      if (key) this.updateExpressionByKey(key);
    };
    this.element.addEventListener("step:change", this._onStepChange);
  ...
  updateExpressionByKey(key) {
    const raw = this.frame3Target?.dataset?.expressionMap;
    if (!raw) return;
    let map;
    try {
      map = JSON.parse(raw);
    ...
    const url = map[key];
    if (url) this.frame3Target.src = url;
  }
  close(event) {
    event?.preventDefault();
    event?.stopPropagation();
  ...
  disconnect() {
    ...
    if (this._onStepChange) this.element.removeEventListener("step:change", this._onStepChange);
...
@hotwired/turbo/dist/turbo.es2017-esm.js:
   ...
   *)
*/
//# sourceMappingURL=/assets/application-62c16e5f.js.map
