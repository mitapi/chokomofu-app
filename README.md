...
- キャラクター1体（MVP時点では静止画。おしゃべり機能用の表情差分も２種類ほど用意する）
- キャラクターとの「おしゃべり」機能（ユーザーが答えを選べる、選択式会話を用意・朝会話のみ実装）
...
１．フロントはRails標準のHotwire(Turbo/Stimulus)を中心に、必要に応じてReactを組み込む想定です。
- React … 会話画面など動的UIが複雑になる部分（MVP後に必要に応じて導入）
...
- Rails + Stimulusで実装予定。React導入も検討。
...
- 最低2体から選択可能にする予定。専用DBを作成してキャラクターのデータを入れる。
